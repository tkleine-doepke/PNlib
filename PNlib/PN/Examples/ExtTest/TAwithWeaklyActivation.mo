...
model TAwithWeaklyActivation
  ...
  PNlib.PN.Components.PC P1(nIn=1, nOut=2, nOutExt = 1, startMarks=4) annotation(Placement(transformation(extent={{-40, 30}, {-20, 50}})));
  PNlib.PN.Components.PC P2(nIn=1, maxMarks=3) annotation(Placement(visible = true, transformation(extent = {{50, 48}, {70, 68}}, rotation = 0)));
  ...
  PNlib.PN.Components.TC T2( nIn=1, nInExt = 1,nOut=1) annotation(Placement(visible = true, transformation(extent = {{20, 48}, {40, 68}}, rotation = 0)));
  PNlib.PN.Components.TC T3(nIn=1, maximumSpeed=if time > 3 then 2 else 0) annotation(Placement(visible = true, transformation(extent = {{20, 12}, {40, 32}}, rotation = 0)));
equation
