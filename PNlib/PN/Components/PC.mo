...
protected
  ...
  PNlib.PN.Blocks.enablingInCon enableIn(active=activeIn, delayPassed=delayPassedIn.anytrue, nIn=nIn, arcWeight=arcWeightIn, t=t_, maxMarks=maxMarks, TAein=enabledByInPlaces and activeIn, enablingType=enablingType, enablingPrio=enablingPrioIn, enablingProb=enablingProbIn, disTransition=disTransitionIn, localSeed=localSeedIn, globalSeed=settings.globalSeed);
  PNlib.PN.Blocks.enablingOutCon enableOut(delayPassed=delayPassedOut.anytrue, nOut=nOut, arcWeight=arcWeightOut, t=t_, minMarks=minMarks, TAout=activeOut, enablingType=enablingType, enablingPrio=enablingPrioOut, enablingProb=enablingProbOut, disTransition=disTransitionOut, localSeed=localSeedOut, globalSeed=settings.globalSeed);
  //Does any delay passed of a connected transition?
  PNlib.PN.Blocks.anyTrue delayPassedOut(vec=activeOut and disTransitionOut);
  PNlib.PN.Blocks.anyTrue delayPassedIn(vec=activeIn and disTransitionIn);
  //Does any discrete transition fire?
  PNlib.PN.Blocks.anyTrue disMarksOut(vec=fireOut and disTransitionOut);
  PNlib.PN.Blocks.anyTrue disMarksIn(vec=fireIn and disTransitionIn);
  //Is the place fed by input transitions?
  PNlib.PN.Blocks.anyTrue feeding(vec=preFireIn and not disTransitionIn);
  //Is the place emptied by output transitions?"
  PNlib.PN.Blocks.anyTrue emptying(vec=preFireOut and not disTransitionOut);
  //firing sum calculation
  PNlib.PN.Blocks.firingSumCon firingSumIn(fire=preFireIn, arcWeight=arcWeightIn, instSpeed=instSpeedIn, disTransition=disTransitionIn);
  PNlib.PN.Blocks.firingSumCon firingSumOut(fire=preFireOut, arcWeight=arcWeightOut, instSpeed=instSpeedOut, disTransition=disTransitionOut);
  //****BLOCKS END****//
...
public
  PNlib.PN.Interfaces.ConPlaceIn inTransition[nIn](each t=t_,
  each tint=1,
  ...
  PNlib.PN.Interfaces.ConPlaceOut outTransition[nOut](each t = t_,
  each tint=1,
