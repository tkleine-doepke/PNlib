  ...
  for i in 1:nIn loop
    if disPlaceIn[i] then //discrete place
      ...
      if arcType[i]==PNlib.Types.ArcType.RealTestArc then //real test arc
        if tIn[i] <= testValue[i] + PNlib.Constants.almost_eps then
          active := false;
        end if;
        if tIn[i] > testValue[i] + PNlib.Constants.almost_eps and fed[i] and not normalArc[i] then  //weakly input active??
          weaklyInputActiveVec[i] := true;
        end if;
      ...
      elseif arcType[i]==PNlib.Types.ArcType.RealInhibitorArc and (tIn[i]  >=  testValue[i]-PNlib.Constants.almost_eps) then  //real inhibitor arc
        active := false;
      elseif arcType[i]==PNlib.Types.ArcType.InhibitorArc and (tIn[i]  > testValue[i] + PNlib.Constants.almost_eps) then  //inhibitor arc
        active := false;
      end if;
    end if;
  end for;
